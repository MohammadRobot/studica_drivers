...
{
    uint8_t data[8] = {0};
    if (!Titan::Read(GetAddress(RETURN_MOTOR_FREQUENCY), data))
        return 0;
...
{
    uint8_t data[8] = {0};
    if (!Titan::Read(GetAddress(MCU_TEMP), data))
        return 0.0f;
...
{
    uint8_t data[8] = {0};
    if (!Titan::Read(GetAddress(LIMIT_SWITCH), data))
        return false;
    uint8_t index = 0;
    if (direction == 1)
    {
...
std::string Titan::GetSerialNumber()
{
    ...
    if (!Titan::Read(GetAddress(RETURN_WORD_1), data1) ||
...
void Titan::SetSpeed(uint8_t motor, double speedCfg)
{
    ...
    uint8_t inB = 0;
    if (speedCfg == 0.0) {
        // Keep legacy behavior: both high indicates brake/neutral on older Titan firmware.
        inA = 1;
        inB = 1;
